...
##        self.termo = visa.instrument( AG34410A )
##        self.lvdt =visa.Instrument( AG34405A )
        self.termo = visa.instrument( AG34405A )
        self.lvdt = visa.Instrument( AG34410A )
        self.termo.write( 'ABORt' )
        self.lvdt.write( 'ABORt' )
        time.sleep(0.5)
        ...
        time.sleep(0.5)
        # self.lvdt.write( 'SENSe:VOLTage:AC:BAND 3' )
       # self.lvdt.write( 'SENSe:VOLTage:AC 10' )
        time.sleep(0.5)
        
       # self.termo.write('SENSe:VOLT:APER 0.06')
        time.sleep( 0.5 )
        
        self.termo.write( 'INIT' )
        self.lvdt.write('INIT')
        time.sleep(0.5)
    ...
    def run ( self ):
        ...
        while not self.exiting:
            ...
            self.termo.write( '*CLS' )
            time.sleep(0.1)
            print 'mido:', contador, ' en:', self.time_stamp, ' seg'
            lvdt_data = self.lvdt.ask('READ?')
            print lvdt_data
            temp_data = self.termo.ask( 'READ?' )
            print temp_data
            
            lvdt_cond=0
            termo_cond= 0
            print 'mando primer OPC?'
            lvdt_cond=self.lvdt.ask('*OPC?')
            termo_cond=self.termo.ask( '*OPC?' )
            print 'LVDT:',lvdt_cond, 'TERMO: ', termo_cond
...
#             print 'Pasar LVDT a mm'
            lvdt_data=float (lvdt_data)*1.7926 #mm/V
            
#             print 'Pasar de milivoltios a gracdos centigrado el valor leido por el multimetro'
...
#             print 'Envisar senial'
        ...
        self.termo.write( 'DISP:WIND2:TEXT:CLEAR' )
        fsock.close()
        self.exit()
...
#-----------------------------------------------------------------------------------
    def Dilatometer_test ( self ):
        ...
        if outfile == '':
            ...
            AG34405A = 'USB0::2391::1560::MY50279055::0::INSTR'
