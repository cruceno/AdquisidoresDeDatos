...
#        self.actionSetup.triggered.connect( self.rs232.start )
        ...
        self.vbl_main = QtGui.QVBoxLayout( self.da_main_plot )
        self.da_maincanvas = canvas( self.da_main_plot )
        self.vbl_main.insertWidget( 0, self.da_maincanvas )
        self.da_mainntb = NavigationToolbar( self.da_maincanvas, self.da_main_plot )
        self.vbl_main.insertWidget( 1, self.da_mainntb )

        self.x = np.arange( 1, 1025, 1 )
    def Plotear( self, y ):
    ...
    def on_actionSave_triggered ( self ):
        ...
        if filename:
            ...
            header = header + self.le_red.text() + '\n'
    ...
    def data_show_and_save( self, espectro ):
        ...
        if self.chk_autosave.checkState():
            ...
            header = str ( time() ) + '\n'
            header += self.le_exposuretime.text() + '\n'
            header += self.le_scansnumber.text() + '\n'
            header += self.le_ignoredscans.text() + '\n'
            header += self.le_damode.text() + '\n'
            header += self.le_detectortemp.text()
            header += '\n' + self.le_red.text() + '\n'
            header += self.le_counter.text() + '\n'
            file = file + '.OMAIII'
            fsock = open( file, 'w' )
            fsock.write( header )
